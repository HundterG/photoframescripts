...
{
    static const char *map = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789-_\0";
                                ...
                                const uint8_t *enc, size_t len, size_t *out_len )
{
    ...
    buf = (uint8_t*) malloc( raw_len * sizeof(uint8_t) );
    if( buf ) {
                             ...
                             const uint8_t *raw, size_t len, size_t *out_len )
{
    ...
    buf = (char*) malloc( (enc_len + 1) * sizeof(char) );
    if( buf ) {
